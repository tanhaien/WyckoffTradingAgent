...
| A股实证入场 | confirmed 信号过滤、弱水温软仓位、条件持有与评分校准 | 默认回测 A/M/O/P/Q/R，未验证前不改变生产漏斗 |
...
2026-07-22 的 A/F/G/H/J/K 复验使用不递补口径。F/G 在牛、熊、近期三段全部显著弱于 A，证明 EVR 与
SOS 不能全局删除；J 的 Spring 强确认同样没有胜出。H 仅在 2020 牛段从 +2.65% 改善到 +16.73%，近期
反而从 -7.94% 恶化到 -9.04%。K 把近期亏损压到 -0.72%、最大回撤压到 -7.20%，但牛段收益降到
-7.67%，且近期只有 9 笔现金成交。参数稳定性为 0 个稳定邻居，walk-forward 为 0/8 个正收益测试窗，
所有组继续停留在研究态。

2026-07-23 的 A/L/M/N 四周期对比显示：L、N 分别以平均 -20.20pp、-14.25pp 的收益差显著弱于 A，
信号族轮转应退出默认算力。M 与 A 的股票集合相同，仅改变研究仓位，却在四个时期全部改善，平均提升
+4.65pp 且没有回撤恶化；但近期 6 个月和 2 个月仍分别亏 -6.78%、-6.27%。参数网格最稳锚点
`probe_add / 15日 / -8% / +18%` 的三时期平均仍为 -0.49%，3 个邻居全部不稳定，walk-forward 仍是
0/8 个正收益测试窗，因此 M 只作为下一轮研究基线，不晋级生产。

同轮逐笔诊断显示，近期最高分四分位平均 -5.91%，最低分四分位反而 +0.48%，原始分不能解释为上涨概率；
NEUTRAL Spring 在牛、熊、近期样本均为负；近期 CAUTION Spring/SOS 的 10 日结果优于 15 日，但其它
时期并不完全一致。下一轮默认对比改为 `A/M/O/P/Q/R`：O 在 M 上不递补过滤 NEUTRAL Spring；P 在 M
上把 CAUTION Spring/SOS 最长持有缩到 10 日；Q 在 M 上对 NEUTRAL Spring -2 分、SOS +1 分做研究
校准；R 组合 O 与 P。评分偏置、条件持有和仓位乘数只存在于回测，不进入生产漏斗、Step3 或 OMS。
汇总报告先计算 M 相对 A 的效果，再计算 O/P/Q/R 相对 M 的增量，避免把 M 的仓位改善重复归因给后续分支。
