...
| A股实证入场 | confirmed 信号族均衡排序与弱水温软仓位 | 默认回测 A/L/M/N，未验证前不改变生产漏斗 |
...
下一轮默认对比固定为 `A/L/M/N`，并补回 `recent_2m`：L 不把历史收益数值写成分数权重，只在每个信号族
内部按原分排序，再让可用信号族按交易日轮转，验证“原始分量纲不可比且单一信号挤占仓位”的假设；M 保留
K 的水温×信号映射，但将原来的硬删除改为 25%～50% 仓位；N 组合 L 与 M。仓位乘数只存在于回测
TradeRecord 和现金组合预算，不进入生产漏斗、Step3 或 OMS。
