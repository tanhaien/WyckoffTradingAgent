...
默认并行运行 A/L/M/N：A 为基线；L 将 confirmed 候选族内按原分排序、族间按交易日轮转，避免不同量纲
的 Spring/SOS/EVR 原始分长期抢占全部仓位；M 将上一轮 K 的弱水温硬删除改为 25%～50% 研究仓位；N
组合 L 与 M。任务覆盖近期 2 个月、近期 6 个月、2020 牛段和 2022 熊段，并关闭动态归因调权。研究开关默认
不改变生产策略，只有跨周期消融和 walk-forward 证据通过后才进入推广评审。
...
经典 B-E 与历史 F-K 仍可手动复验；所有组的持仓退出仍统一使用固定 SL/TP/持有期，因此该报告不能被
解释为动态卖出规则已经完成回测。上轮 F/G/J 被跨周期结果否决，H/K 只显示状态依赖，均未进入生产。
