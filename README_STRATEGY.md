...
默认并行运行 A/F/G/H/J/K：A 为基线；F/G 是锁定基线 Top-N 后的 EVR、EVR+SOS 纯消融；H 增加
NEUTRAL 广度确认；J 增加 Spring 强价格确认；K 按市场水温限制信号族。消融任务关闭动态归因调权，
避免外部策略权重掩盖规则本身贡献。研究开关默认不改变生产策略，只有跨周期消融和 walk-forward 证据通过
后才进入推广评审。
...
经典 B-E 仍可手动复验，其中 B 衡量 Upthrust 当日对新候选的 veto 价值；所有组的持仓退出仍统一使用
固定 SL/TP/持有期，因此该报告不能被解释为动态卖出规则已经完成回测。历史 I 人工先验重排已被三周期
结果否决，不再进入可执行矩阵。
